...
func (n *node) removePeer(peer *peer) {
	...
	select {
	case n.events <- &Event{eventType: EventExit, sender: peer.identity, name: peer.name, time: time.Now()}:
	default:
		if n.verbose {
...
func (n *node) joinPeerGroup(peer *peer, name string) *group {
	...
	select {
	case n.events <- &Event{eventType: EventJoin, sender: peer.identity, name: peer.name, group: name, time: time.Now()}:
	default:
		if n.verbose {
...
func (n *node) leavePeerGroup(peer *peer, name string) *group {
	...
	select {
	case n.events <- &Event{eventType: EventLeave, sender: peer.identity, name: peer.name, group: name, time: time.Now()}:
	default:
		if n.verbose {
...
func (n *node) recvFromPeer(transit msg.Transit) {
	...
	case *msg.Whisper:
		...
		select {
		case n.events <- &Event{eventType: EventWhisper, sender: identity, name: peer.name, msg: m.Content, time: time.Now()}:
		default:
			if n.verbose {
	...
	case *msg.Shout:
		...
		select {
		case n.events <- &Event{eventType: EventShout, sender: identity, name: peer.name, group: m.Group, msg: m.Content, time: time.Now()}:
		default:
			if n.verbose {
